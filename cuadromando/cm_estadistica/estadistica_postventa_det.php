...
$query_estadistica_postventa = sprintf("SELECT *, min(date_ingreso)as fechainicio, max(date_ingreso)as fechatermino, SUM(cantidad_ingresadas)as oping, SUM(cantidad_pendientes)as oppen, SUM(cantidad_cursadas)as opcur, SUM(cantidad_reparadas_esp)as oprepes, SUM(cantidad_reparadas_ope)as opresop, (((SUM(cantidad_reparadas_esp)+SUM(cantidad_reparadas_ope))/SUM(cantidad_ingresadas))*100)as pctrep, SUM(cantidad_urgentes)as opurg, SUM(cantidad_fuerahora)as opfh, ((SUM(cantidad_urgentes)/SUM(cantidad_ingresadas))*100)as pcturg, ((SUM(cantidad_fuerahora)/SUM(cantidad_ingresadas))*100)as pctfh FROM estadistica_postventa WHERE date_ingreso BETWEEN %s AND %s GROUP BY especialista_curse ORDER BY pct_contesta DESC", GetSQLValueString($colname_estadistica_postventa, "date"),GetSQLValueString($colname1_estadistica_postventa, "date"));
