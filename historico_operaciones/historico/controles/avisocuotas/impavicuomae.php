...
$maxRows_avisocuotas = 10;
$pageNum_avisocuotas = 0;
if (isset($_GET['pageNum_avisocuotas'])) {
  $pageNum_avisocuotas = $_GET['pageNum_avisocuotas'];
}
...
$query_avisocuotas = sprintf("SELECT * FROM oppre WHERE nro_operacion = %s and evento = %s ORDER BY id DESC", GetSQLValueString($colname_avisocuotas, "text"),GetSQLValueString($colname1_avisocuotas, "text"));
$query_limit_avisocuotas = sprintf("%s LIMIT %d, %d", $query_avisocuotas, $startRow_avisocuotas, $maxRows_avisocuotas);
$avisocuotas = mysqli_query($historico_goc, $query_limit_avisocuotas) or die(mysqli_error($historico_goc));
...
if (isset($_GET['totalRows_avisocuotas'])) {
  ...
  $all_avisocuotas = mysqli_query($historico_goc, $query_avisocuotas);
...
if (!empty($_SERVER['QUERY_STRING'])) {
  ...
  foreach ($params as $param) {
    if (stristr($param, "pageNum_avisocuotas") == false && 
        stristr($param, "totalRows_avisocuotas") == false) {
...
$queryString_avisocuotas = sprintf("&totalRows_avisocuotas=%d%s", $totalRows_avisocuotas, $queryString_avisocuotas);
?>
...
<table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
  ...
  <?php do { ?>
  <tr valign="middle">
    <td align="center"><a href="impavicuodet.php?recordID=<?php echo $row_avisocuotas['id']; ?>"> <img src="../../../../imagenes/ICONOS/impresora_2.jpg" width="27" height="21" border="0"></a></div></td>
    <td align="center"><span class="respuestacolumna_rojo"><?php echo $row_avisocuotas['nro_operacion']; ?></span>      </div></td>
    <td align="center"><?php echo $row_avisocuotas['fecha_ingreso']; ?></div></td>
    <td align="center"><?php echo $row_avisocuotas['rut_cliente']; ?></div></td>
    <td align="left"><?php echo $row_avisocuotas['nombre_cliente']; ?> </td>
    <td align="center"><?php echo $row_avisocuotas['evento']; ?></div></td>
    </tr>
...
<table border="0" width="50%" align="center">
  <tr>
    <td width="23%" align="center"><?php if ($pageNum_avisocuotas > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_avisocuotas=%d%s", $currentPage, 0, $queryString_avisocuotas); ?>">Primero</a>
        <?php } // Show if not first page ?>
    </td>
    <td width="31%" align="center"><?php if ($pageNum_avisocuotas > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_avisocuotas=%d%s", $currentPage, max(0, $pageNum_avisocuotas - 1), $queryString_avisocuotas); ?>">Anterior</a>
        <?php } // Show if not first page ?>
    </td>
...
<table width="95%"  border="0" align="center">
  <tr>
    <td align="right" valign="middle"><a href="../../../historico.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image2','','../../../../imagenes/Botones/boton_volver_2.jpg',1)"><img src="../../../../imagenes/Botones/boton_volver_1.jpg" alt="Volver" name="Image2" width="80" height="25" border="0"></a></td>
  </tr>
</table>
</body>
</html>
