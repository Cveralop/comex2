...
if (isset($_GET['fecha_ingreso'])) {
  $colname1_impresion = (get_magic_quotes_gpc()) ? $_GET['fecha_ingreso'] : addslashes($_GET['fecha_ingreso']);
}
$colname2_impresion = "zzz";
if (isset($_GET['rut_cliente'])) {
  $colname2_impresion = (get_magic_quotes_gpc()) ? $_GET['rut_cliente'] : addslashes($_GET['rut_cliente']);
}
$colname3_impresion = "1";
if (isset($_GET['evento'])) {
  $colname3_impresion = (get_magic_quotes_gpc()) ? $_GET['evento'] : addslashes($_GET['evento']);
}
$colname_impresion = "1";
if (isset($_GET['especialista_curse'])) {
  $colname_impresion = (get_magic_quotes_gpc()) ? $_GET['especialista_curse'] : addslashes($_GET['especialista_curse']);
}
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_impresion = sprintf("SELECT * FROM oppre WHERE especialista_curse = '%s' and fecha_ingreso = '%s' and rut_cliente LIKE '%%%s%%' and evento = '%s' ORDER BY id DESC", $colname_impresion,$colname1_impresion,$colname2_impresion,$colname3_impresion);
...
<form name="form1" method="get" action="">
  <table width="95%"  border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
    ...
    <tr valign="middle">
      <td width="21%" align="right">Especialista / Territorial:</div></td>
      <td width="79%" align="left"><input name="especialista_curse" type="text" class="etiqueta12" id="especialista_curse" value="<?php echo $_SESSION['login'];?>" size="20" maxlength="20" readonly="readonly">
      / 
      <input name="territorial" type="text" class="etiqueta12" id="territorial" value="<?php echo strtoupper($row_impresion['territorial']); ?>" size="50" maxlength="50" readonly="readonly"></td>
    </tr>
    <tr valign="middle">
...
<table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
  ...
  <tr valign="middle">
    ...
    <td align="center"><span class="respuestacolumna_rojo"><?php echo strtoupper($row_impresion['nro_operacion']); ?></span> / <span class="respuestacolumna_rojo"><?php echo strtoupper($row_impresion['nro_operacion_relacionada']); ?></span></td>
    <td align="right"><span class="Estilo8"><?php echo strtoupper($row_impresion['moneda_operacion']); ?></span> <strong><?php echo number_format($row_impresion['monto_operacion'], 2, ',', '.'); ?></strong></div></td>
  ...
  <tr valign="middle">
    <td rowspan="2" align="left" valign="top" bgcolor="#999999" class="titulocolumnas">Observaciones:</td>
    <td colspan="3" rowspan="2" align="left" valign="top"><?php echo $row_impresion['obs']; ?></td>
    ...
    <td colspan="3" rowspan="2" align="left" valign="top">Tipo Cambio <span class="respuestacolumna_azul"><?php echo $row_impresion['tipocambio']; ?></span> // Origen Fondos <span class="respuestacolumna_azul"><?php echo $row_impresion['origen_fondos']; ?></span>// Destino Fondos <span class="respuestacolumna_azul"><?php echo $row_impresion['destino_fondos']; ?></span>// Cuotas <span class="respuestacolumna_azul"><?php echo $row_impresion['cuotas']; ?></span> // Nro Cuotas <span class="respuestacolumna_azul"><?php echo $row_impresion['nro_cuotas']; ?></span> // TT <span class="respuestacolumna_azul"><?php echo $row_impresion['tt']; ?></span> // SPREAD <span class="respuestacolumna_azul"><?php echo $row_impresion['spread']; ?></span> // TASA FINAL <span class="respuestacolumna_azul"><?php echo $row_impresion['tasa_final']; ?></span> // Periodisidad <span class="respuestacolumna_azul"><?php echo $row_impresion['periodisidad']; ?></span></td>
    </tr>
  <tr valign="middle">
    <td align="right" valign="top" bgcolor="#999999" class="titulocolumnas">Mandato / Imp. Operar / Passport:</td>
    <td align="center" valign="top"><?php echo $row_impresion['mandato']; ?> / <?php echo $row_impresion['impedido_operar']; ?> / <span class="respuestacolumna_rojo"><?php echo $row_impresion['cliente_passport']; ?></span></td>
  </tr>
