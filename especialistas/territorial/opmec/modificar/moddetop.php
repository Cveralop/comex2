...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE opmec SET evento=%s, op_web=%s, estado=%s, operador=%s, obs=%s, territorial=%s, moneda_operacion=%s, monto_operacion=%s, valuta=%s, sub_estado=%s, compraventa=%s, tipocambio=%s, reparo_obs=%s, mandato=%s, excepcion=%s, autorizacion_operaciones=%s, autorizacion_especialista=%s, responsable_excepcion=%s, tipo_excepcion=%s, solucion_excepcion=%s, urgente=%s, fuera_horario=%s, nro_folio=%s, verifico_mandato=%s, hizo_llamada=%s WHERE id=%s",
...
$query_DetailRS1 = sprintf("SELECT * FROM opmec  WHERE id = $recordID", $colname_modificacion,$colname1_modificacion,$colname2_modificacion,$colname3_modificacion);
...
<title>Modificaci&oacute;n Instrucciones Envio OP - Detalle</title>
<style type="text/css">
<!--
...
<table width="95%"  border="1" align="center" bordercolor="#FF0000" bgcolor="#FF0000">
  <tr valign="middle">
    <td width="93%" align="left" class="Estilo3">MODIFICAR INSTRUCCIONES ENVIO OP - DETALLE</td>
...
<form action="<?php echo $editFormAction; ?>" method="POST" name="form1">
  <table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
    ...
    <tr valign="baseline">
      ...
      <td align="center" valign="middle"><select name="compraventa" class="etiqueta12" id="compraventa">
        <option value="N/A" <?php if (!(strcmp("N/A", $row_DetailRS1['compraventa']))) {echo "selected=\"selected\"";} ?>>No Aplica</option>
        <option value="Compra." <?php if (!(strcmp("Compra.", $row_DetailRS1['compraventa']))) {echo "selected=\"selected\"";} ?>>Compra</option>
        <option value="Venta." <?php if (!(strcmp("Venta.", $row_DetailRS1['compraventa']))) {echo "selected=\"selected\"";} ?>>Venta</option>
      </select></td>
...
No</td>
      <td align="right" valign="middle">OP Via WEB:</td>
      <td align="center" valign="middle"><label>Si</label>
        <input <?php if (!(strcmp($row_DetailRS1['op_web'],"Si"))) {echo "checked=\"checked\"";} ?> name="op_web" type="radio" id="op_web_0" value="Si">
        <label> No</label>
      <input <?php if (!(strcmp($row_DetailRS1['op_web'],"No"))) {echo "checked=\"checked\"";} ?> name="op_web" type="radio" id="op_web_1" value="No"></td>
    </tr>
    <tr valign="middle">
    ...
    <tr valign="middle">
      ...
      <span class="textfieldRequiredMsg">Se necesita un valor.</span><span class="textfieldInvalidFormatMsg">Formato no v�lido.</span></span><span class="textfieldRequiredMsg">Se necesita un valor.</span><span class="textfieldInvalidFormatMsg">Formato no v&aacute;lido.</span></span><span class="rojopequeno">(aaaa-mm-dd)</span></td>
    </tr>
    <tr valign="middle">
      <td align="right" bgcolor="#999999">Se Verifico Mandato:</td>
      <td align="center" bgcolor="#999999"><select name="verifico_mandato" class="etiqueta12" id="verifico_mandato">
        <option value="N/A" <?php if (!(strcmp("N/A", $row_DetailRS1['verifico_mandato']))) {echo "selected=\"selected\"";} ?>>N/A</option>
        <option value="Si" <?php if (!(strcmp("Si", $row_DetailRS1['verifico_mandato']))) {echo "selected=\"selected\"";} ?>>Si</option>
        <option value="No" <?php if (!(strcmp("No", $row_DetailRS1['verifico_mandato']))) {echo "selected=\"selected\"";} ?>>No</option>
      </select></td>
      <td align="right" bgcolor="#999999">Se Realizo Llamada:</td>
      <td align="center" bgcolor="#999999"><select name="hizo_llamada" class="etiqueta12" id="hizo_llamada">
        <option value="N/A" <?php if (!(strcmp("N/A", $row_DetailRS1['hizo_llamada']))) {echo "selected=\"selected\"";} ?>>N/A</option>
        <option value="Si" <?php if (!(strcmp("Si", $row_DetailRS1['hizo_llamada']))) {echo "selected=\"selected\"";} ?>>Si</option>
        <option value="No" <?php if (!(strcmp("No", $row_DetailRS1['hizo_llamada']))) {echo "selected=\"selected\"";} ?>>No</option>
      </select></td>
    </tr>
    <tr valign="middle">
...
var sprytextfield1 = new Spry.Widget.ValidationTextField("sprytextfield1", "currency", {validateOn:["blur"], hint:"0.00"});
var sprytextfield2 = new Spry.Widget.ValidationTextField("sprytextfield2", "date", {format:"yyyy-mm-dd", validateOn:["blur"]});
var sprytextarea3 = new Spry.Widget.ValidationTextarea("sprytextarea3", {counterId:"countsprytextarea3", validateOn:["blur"], counterType:"chars_remaining", minChars:0, maxChars:450, isRequired:false});
