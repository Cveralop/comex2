...
$colname2_nrooperacion = "zzzxxx";
if (isset($_GET['referencia'])) {
  $colname2_nrooperacion = $_GET['referencia'];
}
$colname_nrooperacion = "zzzxxx";
if (isset($_GET['nro_operacion'])) {
  $colname_nrooperacion = $_GET['nro_operacion'];
}
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_nrooperacion = sprintf("SELECT * FROM opcce WHERE nro_operacion LIKE %s and evento = %s and referencia LIKE %s ORDER BY date_ingreso DESC", GetSQLValueString("%" . $colname_nrooperacion . "%", "text"),GetSQLValueString($colname1_nrooperacion, "text"),GetSQLValueString("%" . $colname2_nrooperacion . "%", "text"));
