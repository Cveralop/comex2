...
$query_DetailRS1 = sprintf("SELECT opbga.*,(usuarios.nombre)as ne, time(date_espe)as esp FROM opbga, usuarios WHERE opbga.id = $recordID and especialista_curse = %s and (opbga.especialista_curse = usuarios.usuario) ORDER BY opbga.id desc ", GetSQLValueString($colname_DetailRS1, "text"));
