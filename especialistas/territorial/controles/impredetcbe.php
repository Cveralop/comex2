...
$query_DetailRS1 = sprintf("SELECT opcbe.*,(usuarios.nombre)as ne, time(date_espe)as esp FROM opcbe, usuarios WHERE opcbe.id = $recordID and especialista_curse = %s and (opcbe.especialista_curse = usuarios.usuario) ORDER BY opcbe.id desc ", GetSQLValueString($colname_DetailRS1, "text"));
