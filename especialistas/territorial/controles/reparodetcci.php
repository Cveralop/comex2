...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE opcci SET estado=%s, sub_estado=%s WHERE id=%s",
                       GetSQLValueString($_POST['estado'], "text"),
                       GetSQLValueString($_POST['estado'], "text"),
                       GetSQLValueString($_POST['id'], "int"));
  mysqli_select_db($comercioexterior, $database_comercioexterior);
  $Result1 = mysqli_query($comercioexterior, $updateSQL) or die(mysqli_error($comercioexterior));
  $updateGoTo = "reingresocci.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname1_DetailRS1 = "Reparada.";
if (isset($_GET['estado'])) {
  $colname1_DetailRS1 = $_GET['estado'];
}
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_DetailRS1 = sprintf("SELECT * FROM opcci WHERE id = %s and estado = %s", GetSQLValueString($colname_DetailRS1, "int"),GetSQLValueString($colname1_DetailRS1, "text"));
...
$query_DetailRS1 = sprintf("SELECT * FROM opcci  WHERE id = $recordID and estado = 'Reparada.'", $colname_reparos);
$DetailRS1 = mysqli_query($comercioexterior, $query_DetailRS1) or die(mysqli_error($comercioexterior));
$row_DetailRS1 = mysqli_fetch_assoc($DetailRS1);
$totalRows_DetailRS1 = mysqli_num_rows($DetailRS1);
