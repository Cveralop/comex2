...
$query_DetailRS1 = sprintf("SELECT opcex.*,(usuarios.nombre)as ne, time(date_espe)as esp FROM opcex, usuarios WHERE opcex.id = $recordID and especialista_curse = %s and (opcex.especialista_curse = usuarios.usuario) ORDER BY opcex.id desc ", GetSQLValueString($colname_DetailRS1, "text"));
