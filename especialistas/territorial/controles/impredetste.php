...
$query_fuerahorario = sprintf("SELECT * FROM fuerahorario_territorial WHERE depto = %s", GetSQLValueString($colname_fuerahorario, "int"));
...
$query_DetailRS1 = sprintf("SELECT opste.*,(usuarios.nombre)as ne, time(date_espe)as esp FROM opste, usuarios WHERE opste.id = $recordID and especialista_curse = %s and (opste.especialista_curse = usuarios.usuario) ORDER BY opste.id desc ", GetSQLValueString($colname_DetailRS1, "text"));
