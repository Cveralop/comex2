...
$maxRows_ingnro = 10;
$pageNum_ingnro = 0;
if (isset($_GET['pageNum_ingnro'])) {
  $pageNum_ingnro = $_GET['pageNum_ingnro'];
}
$startRow_ingnro = $pageNum_ingnro * $maxRows_ingnro;
$colname_ingnro = "-1";
...
$query_limit_ingnro = sprintf("%s LIMIT %d, %d", $query_ingnro, $startRow_ingnro, $maxRows_ingnro);
$ingnro = mysqli_query($comercioexterior, $query_limit_ingnro) or die(mysqli_error($comercioexterior));
$row_ingnro = mysqli_fetch_assoc($ingnro);
if (isset($_GET['totalRows_ingnro'])) {
  $totalRows_ingnro = $_GET['totalRows_ingnro'];
} else {
  $all_ingnro = mysqli_query($comercioexterior, $query_ingnro);
  $totalRows_ingnro = mysqli_num_rows($all_ingnro);
}
$totalPages_ingnro = ceil($totalRows_ingnro/$maxRows_ingnro)-1;
$queryString_ingape = "";
...
if (!empty($_SERVER['QUERY_STRING'])) {
  ...
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_ingnro") == false && 
        stristr($param, "totalRows_ingnro") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_ingnro = "&" . htmlentities(implode("&", $newParams));
  }
}
...
<form name="form2" method="get" action="">
  <table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
    <tr>
      <td colspan="2" align="left" valign="middle" bgcolor="#999999"><img src="../../../../imagenes/GIF/notepad.gif" alt="" width="19" height="21" border="0"><span class="subtitulopaguina">Ingreso Simple por Nro Operaci&oacute;n</span></td>
    </tr>
    <tr>
...
<table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
  <tr bgcolor="#999999">
    <td align="center" valign="middle" class="titulocolumnas">Ingresar Env&iacute;o OP</td>
  ...
  <tr>
    ...
    <td align="center" valign="middle"><?php echo strtoupper($row_ingape['rut_cliente']); ?></td>
...
<?php if ($totalRows_ingnro > 0) { // Show if recordset not empty ?>
  <table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
    <tr>
      <td align="center" valign="middle" class="titulocolumnas">Ingresar Envio OP</td>
      <td align="center" valign="middle" class="titulocolumnas">Rut Cliente</td>
      <td align="center" valign="middle" class="titulocolumnas">Nombre Cliente</td>
    ...
    <?php do { ?>
      <tr>
        ...
        <td align="center" valign="middle"><?php echo strtoupper($row_ingnro['rut_cliente']); ?> </td>
        <td align="left" valign="middle"><?php echo strtoupper($row_ingnro['nombre_cliente']); ?> </td>
  ...
  <table width="50%" border="0" align="center">
    <tr>
      <td><?php if ($pageNum_ingnro > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_ingnro=%d%s", $currentPage, 0, $queryString_ingnro); ?>">Primero</a>
        <?php } // Show if not first page ?></td>
      <td><?php if ($pageNum_ingnro > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_ingnro=%d%s", $currentPage, max(0, $pageNum_ingnro - 1), $queryString_ingnro); ?>">Anterior</a>
        <?php } // Show if not first page ?></td>
      <td><?php if ($pageNum_ingnro < $totalPages_ingnro) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_ingnro=%d%s", $currentPage, min($totalPages_ingnro, $pageNum_ingnro + 1), $queryString_ingnro); ?>">Siguiente</a>
...
<table width="95%"  border="0" align="center">
  <tr>
    <td align="right" valign="middle"><a href="../opiiib5.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image6','','../../../../imagenes/Botones/boton_volver_2.jpg',1)"><img src="../../../../imagenes/Botones/boton_volver_1.jpg" alt="Volver" name="Image6" width="80" height="25" border="0"></a></div></td>
  </tr>
</table>
</body>
</html>
