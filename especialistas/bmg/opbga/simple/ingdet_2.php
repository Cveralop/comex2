...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO opbga (rut_cliente, nombre_cliente, fecha_ingreso, date_ingreso, evento, obs, especialista_curse, moneda_operacion, monto_operacion, date_preingreso, date_espe, estado_visacion, mandato, urgente) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_DetailRS1 = sprintf("SELECT * FROM opbga WHERE rut_cliente = %s", GetSQLValueString($colname_DetailRS1, "text"));
...
mysqli_select_db($comercioexterior, $database_comercioexterior);
$recordID = $_GET['recordID'];
$query_DetailRS1 = sprintf("SELECT * FROM opbga WHERE id = $recordID"); //, $colname_ingape
...
<option value="ZAR" <?php if (!(strcmp("ZAR", $row_DetailRS1['moneda_operacion']))) {echo "selected=\"selected\"";} ?>>ZAR</option>
            ...
            <option value="UF" <?php if (!(strcmp("UF", $row_DetailRS1['moneda_operacion']))) {echo "selected=\"selected\"";} ?>>UF</option>
          </select> 
