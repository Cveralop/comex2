...
$queryString_nominamandatos = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_nominamandatos") == false && 
        stristr($param, "totalRows_nominamandatos") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_nominamandatos = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_nominamandatos = sprintf("&totalRows_nominamandatos=%d%s", $totalRows_nominamandatos, $queryString_nominamandatos);
?>
