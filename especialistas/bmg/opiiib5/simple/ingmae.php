...
$query_ingape = sprintf("SELECT * FROM cliente WHERE rut_cliente LIKE '%s%%' ORDER BY rut_cliente ASC", $colname_ingape);
...
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_ingnro = sprintf("SELECT * FROM optbc WHERE nro_operacion = %s", GetSQLValueString($colname_ingnro, "text"));
$query_limit_ingnro = sprintf("%s LIMIT %d, %d", $query_ingnro, $startRow_ingnro, $maxRows_ingnro);
$ingnro = mysqli_query($comercioexterior, $query_limit_ingnro) or die(mysqli_error());
...
<form name="form2" method="get" action="">
  <table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
    <tr>
      <td colspan="2" align="left" bgcolor="#999999"><img src="../../../../imagenes/GIF/notepad.gif" width="19" height="21"><span class="subtitulopaguina">Ingreso Simple por Nro Operaci&oacute;n</span></td>
    </tr>
    <tr>
...
<?php if ($totalRows_ingnro > 0) { // Show if recordset not empty ?>
  ...
  <br>
  Registros del <span class="respuestacolumna_azul"><?php echo ($startRow_ingnro + 1) ?></span> al <span class="respuestacolumna_azul"><?php echo min($startRow_ingnro + $maxRows_ingnro, $totalRows_ingnro) ?></span> de un toptal de <span class="respuestacolumna_azul"><?php echo $totalRows_ingnro ?></span>
  <?php } // Show if recordset not empty ?>
<br>
