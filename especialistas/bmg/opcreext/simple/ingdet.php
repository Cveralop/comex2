...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO opcex (rut_cliente, nombre_cliente, fecha_ingreso, date_ingreso, evento, obs, especialista_curse, moneda_operacion, monto_operacion, tipo_operacion, date_preingreso, date_espe, estado_visacion, mandato, urgente, cantidad) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_DetailRS1 = sprintf("SELECT * FROM opcex WHERE rut_cliente = '%s'", $colname_DetailRS1);
...
$query_DetailRS1 = sprintf("SELECT * FROM cliente  WHERE id = $recordID"); // , $colname_ingape (Dentro de la consulta sin inicializar)
...
window.setTimeout(sentencia+"location.replace(direccion);",milisegundos);
</script> 
...
<form action="<?php echo $editFormAction; ?>" method="POST" name="form1" onSubmit="MM_validateForm('monto_operacion','','RisNum');return document.MM_returnValue">
  <table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
    ...
    <tr valign="baseline">
      ...
      <textarea name="obs" cols="80" rows="4" class="etiqueta12">
      <?php echo $row_DetailRS1['obs']; ?>
      </textarea>
