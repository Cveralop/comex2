...
$query_DetailRS1 = sprintf("SELECT optbc.*,(usuarios.nombre)as ne, time(date_espe)as esp FROM optbc, usuarios WHERE optbc.id = $recordID and especialista_curse = %s and (optbc.especialista_curse = usuarios.usuario) ORDER BY optbc.id desc ", GetSQLValueString($colname_DetailRS1, "text"));
