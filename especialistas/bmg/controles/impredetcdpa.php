...
$query_DetailRS1 = sprintf("SELECT opcdpa.*,(usuarios.nombre)as ne, time(date_espe)as esp FROM opcdpa, usuarios WHERE opcdpa.id = $recordID and especialista_curse = %s and (opcdpa.especialista_curse = usuarios.usuario) ORDER BY opcdpa.id desc ", GetSQLValueString($colname_DetailRS1, "text"));
