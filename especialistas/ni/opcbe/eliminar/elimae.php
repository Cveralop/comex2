...
$query_ingape = sprintf("SELECT * FROM opcbe nolock WHERE rut_cliente LIKE %s and (estado_visacion = %s or estado_visacion = %s) and nro_operacion LIKE %s ORDER BY id DESC", GetSQLValueString("%" . $colname_ingape . "%", "text"),GetSQLValueString($colname1_ingape, "text"),GetSQLValueString($colname2_ingape, "text"),GetSQLValueString("%" . $colname3_ingape . "%", "text"));
