...
$query_modificacion = sprintf("SELECT * FROM opste nolock  WHERE rut_cliente = %s and (estado_visacion = %s or estado_visacion = %s) ORDER BY id DESC", GetSQLValueString($colname_modificacion, "text"),GetSQLValueString($colname1_modificacion, "text"),GetSQLValueString($colname2_modificacion, "text"));
...
<table border="0" width="50%" align="center">
  <tr>
    ...
    <td width="23%" align="center"><?php if ($pageNum_modificacion < $totalPages_modificacion) { // Show if not last page ?>
            <a href="<?php printf("%s?pageNum_modificacion=%d%s", $currentPage, min($totalPages_ingsbte, $pageNum_modificacion + 1), $queryString_modificacion); ?>">Siguiente</a>
