...
$query_DetailRS1 = sprintf("SELECT opcci.*,(usuarios.nombre)as ne, time(date_espe)as esp FROM opcci, usuarios WHERE opcci.id = %s and especialista_curse = %s and (opcci.especialista_curse = usuarios.usuario) ORDER BY opcci.id desc ", GetSQLValueString($colname_DetailRS1, "int"), GetSQLValueString($colname1_DetailRS1, "text"));
