...
$query_DetailRS1 = sprintf("SELECT opmec.*,(usuarios.nombre)as ne, time(date_espe)as esp FROM opmec, usuarios WHERE opmec.id = %s and  especialista_curse = %s and (opmec.especialista_curse = usuarios.usuario) ORDER BY opmec.id desc ", GetSQLValueString($colname_DetailRS1, "int"), GetSQLValueString($colname1_DetailRS1, "text"));
