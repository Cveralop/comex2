...
$query_DetailRS1 = sprintf("SELECT opcce.*,(usuarios.nombre)as ne, time(date_espe)as esp FROM opcce, usuarios WHERE opcce.id = %s and especialista_curse = %s and (opcce.especialista_curse = usuarios.usuario) ORDER BY opcce.id desc ", GetSQLValueString($colname_DetailRS1, "int"), GetSQLValueString($colname1_DetailRS1, "text"));
