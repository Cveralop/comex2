...
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_opcce = sprintf("SELECT opcce.*,(cliente.segmento)as seg, (cliente.sub_segmento)as sub_seg, (cliente.sucursal)as suc, (cliente.oficina)as ofi, (cliente.territorial)as terr, (cliente.zonal)as zo, (cliente.nombre_ejecutivo)as eje, (cliente.ejecutivo)as ejeni, (cliente.especialista)as esp FROM opcce LEFT JOIN cliente ON opcce.rut_cliente=cliente.rut_cliente WHERE opcce.date_ingreso between %s and %s", GetSQLValueString($colname2_opcce, "date"),GetSQLValueString($colname3_opcce, "date"));
