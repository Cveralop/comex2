...
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_mandato = sprintf("SELECT cliente.* FROM opcbi INNER JOIN cliente on opcbi.rut_cliente = cliente.rut_cliente WHERE opcbi.id = %s", GetSQLValueString($colname_mandato, "text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO opcbi (nro_folio, id, rut_cliente, nombre_cliente, ejecutivo_cuenta, ejecutivo_ni, especialista_ni, nombre_oficina, fecha_ingreso, date_ingreso, evento, nro_operacion, obs, especialista_curse, territorial, moneda_operacion, monto_operacion, tipo_operacion, sucursal, date_preingreso, date_espe, estado_visacion, mandato, urgente, cliente_passport) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
<form action="<?php echo $editFormAction; ?>" method="POST" name="form1">
  <table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
    ...
    <tr valign="baseline">
      <td width="19%" align="right" valign="middle">Nro Folio:</td>
...
<option value="ZAR" <?php if (!(strcmp("ZAR", $row_DetailRS1['moneda_operacion']))) {echo "selected=\"selected\"";} ?>>ZAR</option>
  ...
  No</label>
          <label>
          <input name="urgente" type="radio" value="Si">
    Si</label>
          <br>
      </td>
    </tr>
    <tr valign="baseline">
      ...
      <td align="center" valign="middle"><input name="tipo_operacion" type="text" class="etiqueta12" id="tipo_operacion" value="<?php echo $row_DetailRS1['tipo_operacion']; ?>" size="5" maxlength="5" readonly="readonly">        </div></td>
    </tr>
    <tr valign="baseline">
      ...
      <td align="center" valign="middle">
        ...
        / 
      <input name="cliente_passport" type="text" class="respuestacolumna_rojo" id="cliente_passport" value="<?php echo $row_mandato['cliente_passport']; ?>" size="3" maxlength="2"></td>
      <td align="right" valign="middle">Condiciones Especiales:</td>
      <td align="center" valign="middle"><select name="cond_espe" class="etiqueta12" id="cond_espe">
        <option value="Si" <?php if (!(strcmp("Si", $row_mandato['cond_espe']))) {echo "selected=\"selected\"";} ?>>Si</option>
        <option value="No" <?php if (!(strcmp("No", $row_mandato['cond_espe']))) {echo "selected=\"selected\"";} ?>>No</option>
      </select></td>
    </tr>
    <tr valign="baseline">
      <td align="right" valign="middle">Tipo Condiciones Especiales:</td>
      <td colspan="3" align="left" valign="middle"><textarea name="tipo_cond_espe" cols="85" rows="4" class="etiqueta12" id="tipo_cond_espe"><?php echo $row_mandato['tipo_cond_espe']; ?></textarea></td>
    </tr>
    <tr valign="baseline">
...
<table width="95%"  border="0" align="center">
  <tr>
    <td align="right" valign="middle"><a href="ingmae_m.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image2','','../../../../imagenes/Botones/boton_volver_2.jpg',1)"><img src="../../../../imagenes/Botones/boton_volver_1.jpg" alt="Volver" name="Image2" width="80" height="25" border="0"></a></div></td>
  </tr>
</table>
<script type="text/javascript">
<!--
var sprytextarea1 = new Spry.Widget.ValidationTextarea("sprytextarea1", {minChars:0, maxChars:255, validateOn:["blur"], counterId:"countsprytextarea1", counterType:"chars_remaining", isRequired:false});
