...
$query_DetailRS1 = sprintf("SELECT * FROM opcce nolock WHERE id = %s", GetSQLValueString($colname_DetailRS1, "text"));
...
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_mandato = sprintf("SELECT cliente.* FROM opcce INNER JOIN cliente on opcce.rut_cliente = cliente.rut_cliente WHERE opcce.id = %s", GetSQLValueString($colname_mandato, "text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO opcce (nro_folio, id, rut_cliente, nombre_cliente, ejecutivo_cuenta, ejecutivo_ni, especialista_ni, nombre_oficina, fecha_ingreso, date_ingreso, evento, nro_operacion, obs, especialista_curse, territorial, moneda_operacion, monto_operacion, moneda_documentos, monto_documentos, date_preingreso, date_espe, estado_visacion, mandato, urgente, cliente_passport) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
<option value="ZAR" <?php if (!(strcmp("ZAR", $row_DetailRS1['moneda_operacion']))) {echo "selected=\"selected\"";} ?>>ZAR</option>
  ...
  <input type="hidden" name="moneda_documentos" id="moneda_documentos">
  <input type="hidden" name="monto_documentos" id="monto_documentos">
