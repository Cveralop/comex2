...
$colname2_ingvarios = "-1";
if (isset($_GET['referencia'])) {
  $colname2_ingvarios = $_GET['referencia'];
}
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_ingvarios = sprintf("SELECT * FROM opcce nolock  WHERE nro_operacion LIKE %s and evento = %s and referencia LIKE %s ORDER BY id DESC", GetSQLValueString("%" . $colname_ingvarios . "%", "text"),GetSQLValueString($colname1_ingvarios, "text"),GetSQLValueString("%" . $colname2_ingvarios . "%", "text"));
...
<table border="0" width="50%" align="center">
        <tr>
            ...
            <td width="23%" align="center"><?php if ($pageNum_ingvarios < $totalPages_ingvarios) { // Show if not last page ?>
                <a
                    href="<?php printf("%s?pageNum_ingvarios=%d%s", $currentPage, $totalPages_ingvarios, $queryString_ingvarios); ?>">último</a>
                <?php } // Show if not last page ?>
            </td>
        </tr>
    </table>
