...
$query_DetailRS1 = sprintf("SELECT * FROM opcex nolock WHERE id = %s", GetSQLValueString($colname_DetailRS1, "text"));
...
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_mandato = sprintf("SELECT cliente.* FROM opcex INNER JOIN cliente on opcex.rut_cliente = cliente.rut_cliente WHERE opcex.id = %s", GetSQLValueString($colname_mandato, "text"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE opcex SET evento=%s, obs=%s, moneda_operacion=%s, monto_operacion=%s, tipo_operacion=%s, date_preingreso=%s, date_espe=%s, mandato=%s, urgente=%s WHERE id=%s",
