...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE opcbe SET evento=%s, estado=%s, asignador=%s, operador=%s, nro_operacion=%s, obs=%s, moneda_operacion=%s, monto_operacion=%s, valuta=%s, sub_estado=%s, date_visa=%s, date_asig=%s, reparo_obs=%s, visador=%s, mandato=%s, estado_visacion=%s, excepcion=%s, autorizacion_operaciones=%s, autorizacion_especialista=%s, responsable_excepcion=%s, tipo_excepcion=%s, solucion_excepcion=%s, urgente=%s, fuera_horario=%s WHERE id=%s",
...
mysqli_select_db($comercioexterior, $database_comercioexterior);
$recordID = $_GET['recordID'];
$query_DetailRS1 = "SELECT * FROM opcbe WHERE id = $recordID";
$DetailRS1 = mysqli_query($comercioexterior, $query_DetailRS1) or die(mysqli_error($comercioexterior));
$row_DetailRS1 = mysqli_fetch_assoc($DetailRS1);
$totalRows_DetailRS1 = mysqli_num_rows($DetailRS1);
?>
  ...
  <table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
    ...
    <tr valign="baseline">
      ...
      <td align="center" valign="middle">        <select name="evento" class="etiqueta12" id="evento">
        ...
        <option value="Solucion Excecpion." <?php if (!(strcmp("Solucion Excecpion.", $row_DetailRS1['evento']))) {echo "selected=\"selected\"";} ?>>Solucion Excepcion</option>
...
Monto Operaci&oacute;n:</td>
    ...
    <tr valign="baseline">
      ...
      <select name="estado" class="etiqueta12" id="estado">
        <option value="Pendiente." selected>Cursada</option>
