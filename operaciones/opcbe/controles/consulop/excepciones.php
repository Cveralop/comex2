...
header("Content-Disposition: attachment;filename=opcbe_excepciones.xls"); 
...
$query_excepciones = sprintf("SELECT opcbe.*,(usuarios.nombre)as ne ,timestampdiff(day,solucion_excepcion,current_timestamp)as dias FROM opcbe, usuarios WHERE estado_excepcion = %s and excepcion = %s and estado = %s and (opcbe.especialista_curse = usuarios.usuario) ORDER BY solucionado ASC", GetSQLValueString($colname_excepciones, "text"),GetSQLValueString($colname1_excepciones, "text"),GetSQLValueString($colname2_excepciones, "text"));
