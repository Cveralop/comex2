...
$query_reparo = sprintf("SELECT * FROM opcdpa nolock WHERE rut_cliente LIKE %s and evento = %s and nro_operacion = %s and sub_estado = 'Reparada.' ", GetSQLValueString($colname_reparo . "%", "text"),GetSQLValueString($colname1_reparo, "text"),GetSQLValueString($colname2_reparo, "text"));
$query_limit_reparo = sprintf("%s LIMIT %d, %d", $query_reparo, $startRow_reparo, $maxRows_reparo); //Error de linea
