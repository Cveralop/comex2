...
header("Content-Disposition: attachment;filename=letras_curstodia.xls"); 
...
$query_DetailRS1 = sprintf("SELECT *, (cliente.sucursal)as suc,(cliente.oficina)as ofi FROM opcbi LEFT JOIN cliente ON opcbi.rut_cliente =  cliente.rut_cliente WHERE estado = %s  and evento = %s and date_curse between %s and %s ORDER BY date_curse ASC", GetSQLValueString($colname_DetailRS1, "text"),GetSQLValueString($colname1_DetailRS1, "text"),GetSQLValueString($colname2_DetailRS1, "date"),GetSQLValueString($colname3_DetailRS1, "date"));
