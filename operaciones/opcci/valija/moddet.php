...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE opcci SET obs=%s, fecha_valija=%s, nro_sobre=%s, despacho_doctos=%s, sucursal=%s, encargado_sucursal=%s, receptor=%s, acuse_recibo=%s WHERE id=%s",
...
$query_DetailRS1 = sprintf("SELECT * FROM opcci  WHERE id = $recordID", $colname_valija,$colname1_valija);
...
<table width="95%"  border="1" align="center" bordercolor="#FF0000" bgcolor="#FF0000">
  <tr>
    <td width="93%" align="left" valign="middle"><span class="Estilo8">MODIFICACI&Oacute;N VALIJA DETALLE CARTA DE CR&Eacute;DITO DE IMPORTACI&Oacute;N</span></td>
...
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" onSubmit="MM_validateForm('fecha_valija','','R','nro_sobre','','R','despacho_doctos','','R','sucursal','','R','encargado_sucursal','','R','receptor','','R','obs','','R');return document.MM_returnValue">
  <table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
    ...
    <tr valign="middle">
      <td align="right" valign="middle">Acuse Recivo: </td>
      <td align="center" valign="middle"> 
      <select name="acuse_recibo" class="etiqueta12" id="acuse_recibo">
        <option value="Pendiente." <?php if (!(strcmp("Pendiente.", $row_DetailRS1['acuse_recibo']))) {echo "selected=\"selected\"";} ?>>Pendiente.</option>
        <option value="Recibido." <?php if (!(strcmp("Recibido.", $row_DetailRS1['acuse_recibo']))) {echo "selected=\"selected\"";} ?>>Recibido.</option>
        <option value="Devueltos." <?php if (!(strcmp("Devueltos.", $row_DetailRS1['acuse_recibo']))) {echo "selected=\"selected\"";} ?>>Devueltos.</option>
      </select>       
        </div>
      </div></td>
