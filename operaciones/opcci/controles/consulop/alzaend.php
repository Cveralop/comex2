...
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_cursada = sprintf("SELECT * FROM opcci WHERE evento LIKE %s and estado LIKE %s and date_ingreso between %s and %s ORDER BY id DESC", GetSQLValueString("%" . $colname_cursada . "%", "text"),GetSQLValueString("%" . $colname1_cursada . "%", "text"),GetSQLValueString($colname2_cursada, "date"),GetSQLValueString($colname3_cursada, "date"));
$cursada = mysqli_query($comercioexterior, $query_cursada) or die(mysqli_error());
$row_cursada = mysqli_fetch_assoc($cursada);
$colname_cursada = "1";
if (isset($_GET['evento'])) {
...
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_cursada = sprintf("SELECT *,(usuarios.nombre)as nombre FROM opcci, usuarios WHERE evento = %s and date_ingreso between %s and %s and (opcci.especialista_curse = usuarios.usuario) ORDER BY date_ingreso DESC", GetSQLValueString($colname_cursada, "text"),GetSQLValueString($colname2_cursada, "date"),GetSQLValueString($colname3_cursada, "date"));
$cursada = mysqli_query($comercioexterior, $query_cursada) or die(mysqli_error());
$row_cursada = mysqli_fetch_assoc($cursada);
...
<table width="95%"  border="1" align="center" bordercolor="#FF0000" bgcolor="#FF0000">
  <tr>
    <td align="left" valign="middle" class="Estilo3"> OPERACIONES CURSADAS (Alzamiento Anticipado - Endoso Anticipado) </td>
...
<form name="form1" method="get" action="">
  <table width="95%"  border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
    <tr valign="middle" bgcolor="#999999">
      <td colspan="4" align="left" valign="middle"><span class="Estilo5"><img src="../../../../imagenes/GIF/notepad.gif" width="19" height="21" border="0"></span><span class="Estilo5">Estado Operaciones Alzamiento - Endoso Anticipado</span></td>
    </tr>
    <tr valign="middle">
...
<br>
  ...
  <table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
    <tr valign="middle" bgcolor="#999999">
      <td colspan="6" align="left"><span class="Estilo5"><img src="../../../../imagenes/GIF/notepad.gif" width="19" height="21" border="0">Total Operaciones <span class="Estilo10"><?php echo $totalRows_cursada ?></span></span></td>
    </tr>
    <tr valign="middle" bgcolor="#999999">
    ...
    <tr valign="middle">
      ...
      <td align="center"><?php echo $row_cursada['fecha_ingreso']; ?> </div>        </div></td>
      <td align="center"><?php echo $row_cursada['rut_cliente']; ?> </div></td>
      <td align="left"><?php echo strtoupper($row_cursada['nombre_cliente']); ?> </div></td>
      <td align="center"><?php echo strtoupper($row_cursada['nombre']); ?> </td>
      <td align="right"><strong><span class="respuestacolumna_rojo"><?php echo strtoupper($row_cursada['moneda_documentos']); ?> </span><span class="respuestacolumna_azul"><?php echo number_format($row_cursada['monto_documentos'], 2, ',', '.'); ?></span></strong>        </div>        </div>        </div></td>
      </tr>
