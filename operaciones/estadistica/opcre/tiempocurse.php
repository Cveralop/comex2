...
$query_tiempocurse = sprintf("SELECT *,timestampdiff(hour,date_espe,date_visa)as dif1,timestampdiff(hour,date_visa,date_asig)as dif2,timestampdiff(hour,date_asig,date_oper)as dif3,timestampdiff(hour,date_oper,date_supe)as dif4 FROM opcex WHERE fecha_ingreso LIKE %s ORDER BY evento ASC", GetSQLValueString("%" . $colname_tiempocurse . "%", "text"));
$tiempocurse = mysqli_query($comercioexterior, $query_tiempocurse) or die(mysqli_error($comercioexterior));
...
<table border="0" width="50%" align="center">
  <tr>
    ...
    <td width="23%" align="center"><?php if ($pageNum_tiempocurse < $totalPages_tiempocurse) { // Show if not last page ?>
      <a href="<?php printf("%s?pageNum_tiempocurse=%d%s", $currentPage, $totalPages_tiempocurse, $queryString_tiempocurse); ?>">&Uacute;ltimo</a>
      <?php } // Show if not last page ?>
    </td>
  </tr>
</table>
