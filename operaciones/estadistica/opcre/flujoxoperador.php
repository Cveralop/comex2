...
$query_flujo = sprintf("SELECT operador,tipo_operacion,evento,date_oper,min(date_oper)as minimo,max(date_oper)as maximo,count(opcex.id)as cantidad,(usuarios.nombre)as nombre FROM opcex, usuarios WHERE date_curse between %s and %s and estado = %s and (opcex.operador = usuarios.usuario) GROUP BY operador,tipo_operacion,evento ORDER BY nombre asc", GetSQLValueString($colname2_flujo, "date"),GetSQLValueString($colname3_flujo, "date"),GetSQLValueString($colname1_flujo, "text"));
...
<table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
  ...
  <tr align="center" valign="middle">
    ...
    <td><span class="respuestacolumna"><?php echo $row_flujo['minimo']; ?></span></td>
    <td><span class="respuestacolumna"><?php echo $row_flujo['maximo']; ?></span></td>
  </tr>
