...
header("Content-Disposition: attachment;filename=opcex_cursadas.xls"); 
...
$query_DetailRS1 = sprintf("SELECT opcex.*,(usuarios.nombre)as ne,(usuarios.segmento)as seg FROM opcex, usuarios WHERE date_ingreso between %s and %s and (opcex.especialista_curse = usuarios.usuario) ORDER BY date_ingreso ASC", GetSQLValueString($colname2_DetailRS1, "date"),GetSQLValueString($colname3_DetailRS1, "date"));
