...
$query_flujo = sprintf("SELECT opcex.*,(usuarios.nombre)as nombre,count(urgente)as cantidad,(usuarios.segmento)as seg FROM opcex, usuarios WHERE date_ingreso between %s and %s and (opcex.especialista_curse = usuarios.usuario) and estado = %s and urgente = %s GROUP BY nombre,evento ORDER BY cantidad desc", GetSQLValueString($colname2_flujo, "date"),GetSQLValueString($colname3_flujo, "date"),GetSQLValueString($colname4_flujo, "text"),GetSQLValueString($colname5_flujo, "text"));
