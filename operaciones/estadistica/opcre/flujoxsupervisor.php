...
$query_flujo = sprintf("SELECT autorizador,tipo_operacion,evento,date_supe,min(date_supe)as minimo,max(date_supe)as maximo,count(opcex.id)as cantidad,(usuarios.nombre)as nombre FROM opcex, usuarios WHERE date_curse between %s and %s and estado = %s and (opcex.autorizador = usuarios.usuario) GROUP BY autorizador,tipo_operacion,evento ORDER BY autorizador asc", GetSQLValueString($colname2_flujo, "date"),GetSQLValueString($colname3_flujo, "date"),GetSQLValueString($colname1_flujo, "text"));
