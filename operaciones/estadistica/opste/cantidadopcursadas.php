...
$query_cursada = sprintf("SELECT tipo_operacion,operador,evento,count(operador)as cursada,(usuarios.nombre)as nombre FROM opste,usuarios WHERE date_curse between %s and %s and estado = %s and (opste.operador = usuarios.usuario) GROUP BY tipo_operacion,evento ORDER BY tipo_operacion,evento ASC", GetSQLValueString($colname2_cursada, "date"),GetSQLValueString($colname3_cursada, "date"),GetSQLValueString($colname1_cursada, "text"));
...
<table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
  ...
  <?php do { ?>
  <tr>
    <td align="center" valign="middle">STAND BY EMITIDA <span class="respuestacolumna"><?php echo strtoupper($row_cursada['tipo_operacion']); ?></span></td>
