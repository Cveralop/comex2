...
$query_flujo = sprintf("SELECT tipo_operacion,operador,nro_operacion,evento,date_oper,min(date_oper)as minimo,max(date_oper)as maximo,count(opste.id)as cantidad,(usuarios.nombre)as nombre FROM opste, usuarios WHERE date_curse between %s and %s and estado = %s and (opste.operador = usuarios.usuario) GROUP BY operador,tipo_operacion,evento ORDER BY operador asc", GetSQLValueString($colname2_flujo, "date"),GetSQLValueString($colname3_flujo, "date"),GetSQLValueString($colname1_flujo, "text"));
