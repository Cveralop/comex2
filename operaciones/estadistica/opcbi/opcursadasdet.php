...
header("Content-Disposition: attachment;filename=opcbi_cursadas.xls"); 
...
$query_DetailRS1 = sprintf("SELECT opcbi.*,(usuarios.nombre)as ne,(usuarios.segmento)as seg FROM opcbi, usuarios WHERE date_ingreso between %s and %s and (opcbi.especialista_curse = usuarios.usuario) ORDER BY date_ingreso ASC", GetSQLValueString($colname2_DetailRS1, "date"),GetSQLValueString($colname3_DetailRS1, "date"));
