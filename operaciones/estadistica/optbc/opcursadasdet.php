...
header("Content-Disposition: attachment;filename=optbc_cursadas.xls"); 
...
$query_DetailRS1 = sprintf("SELECT optbc.*,(usuarios.nombre)as ne,(usuarios.segmento)as seg FROM optbc, usuarios WHERE date_ingreso between %s and %s and (optbc.especialista_curse = usuarios.usuario) ORDER BY date_ingreso ASC", GetSQLValueString($colname2_DetailRS1, "date"),GetSQLValueString($colname3_DetailRS1, "date"));
