...
$query_flujo = sprintf("SELECT oppre.*,count(oppre.id)as cantidad,(usuarios.nombre)as ne,(usuarios.segmento)as seg FROM oppre, usuarios WHERE date_curse between %s and %s and (oppre.especialista_curse = usuarios.usuario) and estado = %s and fuera_horario = %s GROUP BY nombre asc", GetSQLValueString($colname2_flujo, "date"),GetSQLValueString($colname3_flujo, "date"),GetSQLValueString($colname4_flujo, "text"),GetSQLValueString($colname5_flujo, "text"));
