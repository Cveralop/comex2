...
$query_cursada = sprintf("SELECT operador,tipo_operacion,evento,sum(nro_cuotas)as cursada,(usuarios.nombre)as nombre FROM oppre,usuarios WHERE date_curse between %s and %s and estado = %s and (oppre.operador = usuarios.usuario) GROUP BY tipo_operacion,evento ORDER BY tipo_operacion ASC", GetSQLValueString($colname2_cursada, "date"),GetSQLValueString($colname3_cursada, "date"),GetSQLValueString($colname1_cursada, "text"));
