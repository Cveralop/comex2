...
$query_flujo = sprintf("SELECT opmec.*,sum(opmec.cantidad)as cantidad,(usuarios.nombre)as ne,(usuarios.segmento)as seg FROM opmec, usuarios WHERE date_curse between %s and %s and (opmec.especialista_curse = usuarios.usuario) and estado = %s and fuera_horario = %s GROUP BY nombre asc", GetSQLValueString($colname2_flujo, "date"),GetSQLValueString($colname3_flujo, "date"),GetSQLValueString($colname4_flujo, "text"),GetSQLValueString($colname5_flujo, "text"));
