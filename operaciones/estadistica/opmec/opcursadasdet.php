...
header("Content-Disposition: attachment;filename=opmec_cursadas.xls"); 
...
$query_DetailRS1 = sprintf("SELECT opmec.*,(usuarios.nombre)as ne,(usuarios.segmento)as seg FROM opmec, usuarios WHERE date_ingreso between %s and %s and (opmec.especialista_curse = usuarios.usuario) ORDER BY date_ingreso ASC", GetSQLValueString($colname2_DetailRS1, "date"),GetSQLValueString($colname3_DetailRS1, "date"));
