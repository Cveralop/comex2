...
$query_flujo = sprintf("SELECT opcce.*,count(opcce.id)as cantidad,(usuarios.nombre)as ne,(usuarios.segmento)as seg FROM opcce, usuarios WHERE date_curse between %s and %s and (opcce.especialista_curse = usuarios.usuario) and estado = %s and fuero_horario = %s GROUP BY nombre asc", GetSQLValueString($colname2_flujo, "date"),GetSQLValueString($colname3_flujo, "date"),GetSQLValueString($colname4_flujo, "text"),GetSQLValueString($colname5_flujo, "text"));
