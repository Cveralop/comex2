...
$query_flujo = sprintf("SELECT opcbe.*,count(opcbe.id)as cantidad,(usuarios.nombre)as ne,(usuarios.segmento)as seg FROM opcbe, usuarios WHERE date_curse between %s and %s and (opcbe.especialista_curse = usuarios.usuario) and estado = %s and fuero_horario = %s group by nombre asc", GetSQLValueString($colname2_flujo, "date"),GetSQLValueString($colname3_flujo, "date"),GetSQLValueString($colname4_flujo, "text"),GetSQLValueString($colname5_flujo, "text"));
