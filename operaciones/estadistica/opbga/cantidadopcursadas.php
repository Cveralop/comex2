...
$query_cursada = sprintf("SELECT operador,tipo_operacion,count(opbga.id)as cursada,(usuarios.nombre)as nombre FROM opbga,usuarios WHERE date_curse between %s and %s and estado = %s and (opbga.operador = usuarios.usuario) GROUP BY tipo_operacion ORDER BY tipo_operacion ASC", GetSQLValueString($colname2_cursada, "date"),GetSQLValueString($colname3_cursada, "date"),GetSQLValueString($colname1_cursada, "text"));
...
<table width="95%" border="1" align="center" bordercolor="#666666" bgcolor="#CCCCCC">
  ...
  <?php do { ?>
  <tr>
    <td align="center" valign="middle"><span class="respuestacolumna">BOLETA DE GARANTIA <?php echo strtoupper($row_cursada['tipo_operacion']); ?></span></div></td>
