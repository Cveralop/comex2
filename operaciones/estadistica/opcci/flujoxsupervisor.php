...
$query_flujo = sprintf("SELECT autorizador,nro_operacion,evento,date_supe,min(date_supe)as minimo,max(date_supe)as maximo,count(opcci.id)as cantidad,(usuarios.nombre)as nombre FROM opcci, usuarios WHERE date_curse between %s and %s and estado = %s and (opcci.autorizador = usuarios.usuario) GROUP BY autorizador,evento ORDER BY autorizador asc", GetSQLValueString($colname2_flujo, "date"),GetSQLValueString($colname3_flujo, "date"),GetSQLValueString($colname1_flujo, "text"));
