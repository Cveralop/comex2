...
$query_flujo = sprintf("SELECT especialista,evento,excepcion,count(excepcion)as cantidad,(usuarios.nombre)as nombre FROM opcci, usuarios WHERE date_curse between %s and %s and estado = %s and excepcion = %s and (opcci.especialista_curse = usuarios.usuario) GROUP BY especialista_curse,evento ORDER BY cantidad desc", GetSQLValueString($colname2_flujo, "int"),GetSQLValueString($colname3_flujo, "date"),GetSQLValueString($colname1_flujo, "text"),GetSQLValueString($colname4_flujo, "text"));
