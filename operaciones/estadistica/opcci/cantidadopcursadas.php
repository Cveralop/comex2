...
$query_cursada = sprintf("SELECT operador,evento,sum(nro_cuotas)as cursada,(usuarios.nombre)as nombre FROM opcci,usuarios WHERE date_curse between %s and %s and estado = %s and (opcci.operador = usuarios.usuario) GROUP BY evento ORDER BY evento ASC", GetSQLValueString($colname2_cursada, "date"),GetSQLValueString($colname3_cursada, "date"),GetSQLValueString($colname1_cursada, "text"));
