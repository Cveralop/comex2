...
$colname5_flujo = "Si";
if (isset($_GET['fuero_horario'])) {
  $colname5_flujo = $_GET['fuero_horario'];
}
mysqli_select_db($comercioexterior, $database_comercioexterior);
$query_flujo = sprintf("SELECT opcci.*,count(opcci.id)as cantidad,(usuarios.nombre)as ne,(usuarios.segmento)as seg FROM opcci, usuarios WHERE date_curse between %s and %s and (opcci.especialista_curse = usuarios.usuario) and estado = %s and fuero_horario = %s GROUP BY nombre asc", GetSQLValueString($colname2_flujo, "date"),GetSQLValueString($colname3_flujo, "date"),GetSQLValueString($colname4_flujo, "text"),GetSQLValueString($colname5_flujo, "text"));
